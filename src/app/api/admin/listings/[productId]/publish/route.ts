// POST /api/admin/listings/[productId]/publish  { reason?: string }

import { NextResponse } from "next/server";
import { z } from "zod";
import { requireRole } from "@/lib/auth";
import { adminPublishProduct } from "@/lib/admin";
...
export async function POST(req: Request, ctx: { params: Promise<{ productId: string }> }) {
  ...
  try {
    const product = await adminPublishProduct(productId, admin.id, parsed.data.reason);
